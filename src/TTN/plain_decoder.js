...
function decodeUplink(input) {
  ...
  if (input.fPort === 11) {
      // Decoding Victron SmartShunt data
      var i = 0;
      data.battery_voltage = ((input.bytes[i++] << 8) | input.bytes[i++]);
      data.current = ((input.bytes[i++] << 8) | input.bytes[i++]);
      data.power = ((input.bytes[i++] << 8) | input.bytes[i++]);
      data.state_of_charge = input.bytes[i++];
      data.temperature = ((input.bytes[i++] << 8) | input.bytes[i++]);
      data.time_to_go = ((input.bytes[i++] << 8) | input.bytes[i++]);
  }
